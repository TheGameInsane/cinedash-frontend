...
function getGenreDistribution(entries: { genre: number[], rating: number }[]) {
  ...
  return Object.entries(genreCount)
    .map(([genreId, count]) => ({ genreId, percentage: Number.parseFloat(((count / sum) * 100).toFixed(2)) }))
    .sort((a, b) => b.percentage - a.percentage)
    .slice(0, 6);
}
const valueFormatter = (item: { value: number }) => `${item.value}%`;

function GenreBreakdown({ topGenres, genreMap, accentColor, label }: {
    ...
    <div>
      ...
      <div className="space-y-3">
        <PieChart
          series={[
            {
              data: topGenres.map((entry) => ({ value: entry.percentage, label: genreMap[Number(entry.genreId)] })),
...
export default function Dashboard() {
  ...
  const movieHistory = (user.watchHistory || []).filter(w => w.type === 'movie');
  ...
  const avgRating = totalWatched > 0
    ? (user.watchHistory.reduce((sum, m) => sum + m.rating, 0) / totalWatched).toFixed(1)
    : '0';
  ...
  const overallTopGenre = Object.entries(allGenreCount).sort(([, a], [, b]) => b - a)[0];
  const topGenreName = overallTopGenre
    ? (MOVIE_GENRE_MAP[Number(overallTopGenre[0])] || TV_GENRE_MAP[Number(overallTopGenre[0])] || 'Unknown')
    : '—';
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger-children">
          ...
          <div className="glass rounded-xl p-6 group hover:border-amber-500/20 transition-all duration-300">
            ...
            <p className="text-3xl font-bold">{avgRating}<span className="text-lg text-slate-500"> / 10</span></p>
          </div>
